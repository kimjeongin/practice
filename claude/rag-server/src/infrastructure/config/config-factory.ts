...
export interface ConfigProfile {
  name: string;
  description: string;
  config: Partial<AdvancedServerConfig>;
  extends?: string;
}
...
export class ConfigFactory {
   ...
   */
  static createDevelopmentConfig(): AdvancedServerConfig {
   ...
   */
  static createProductionConfig(): AdvancedServerConfig {
    const baseConfig = ConfigFactory.createBaseConfig();
    ...
    return {
      ...
      vectorStore: {
        ...
        config: {
          ...
          qdrantCollectionName: process.env['QDRANT_COLLECTION'] || 'documents',
        }
      },
      pipeline: {
   ...
   */
  static createTestConfig(): AdvancedServerConfig {
    const baseConfig = ConfigFactory.createBaseConfig();
    ...
    return {
      ...
      nodeEnv: 'test',
      logLevel: 'error',
      documentsDir: resolve('./test/documents'),
      dataDir: resolve('./test/.data'),
      vectorStore: {
        provider: 'faiss',
        config: {
          indexPath: resolve('./test/.data/vectors'),
        }
      },
      pipeline: {
   ...
   */
  getConfig(profileName: string): AdvancedServerConfig {
    const profile = this.profiles.get(profileName);
    if (!profile) {
      throw new Error(`Configuration profile '${profileName}' not found`);
    }
    
    let config = ConfigFactory.createBaseConfig() as AdvancedServerConfig;
    ...
    return { ...config, ...profile.config } as AdvancedServerConfig;
  }
  
  /**
   * Get configuration based on current environment
   */
  static getCurrentConfig(): AdvancedServerConfig {
   ...
   */
  static validateConfig(config: AdvancedServerConfig): void {
    const errors: string[] = [];
    ...
    if (config.vectorStore.provider === 'qdrant' && !config.vectorStore.config.qdrantUrl) {
