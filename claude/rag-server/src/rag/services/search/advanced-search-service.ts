/**
 * Advanced Search Service - Pipeline-based Modern RAG System
 * Implements 2025 best practices: Query processing, hybrid search, reranking
 */
...
import { VectorStoreProvider } from '@/infrastructure/vectorstore/abstractions/vector-store-interface.js';
...
export class AdvancedSearchService implements ISearchService {
  ...
  async search(query: string, options: SearchOptions = {}): Promise<SearchResult[]> {
    ...
    const endTiming = startTiming('advanced_search', { 
      ...
      searchType,
      component: 'AdvancedSearchService'
    });
  ...
  private async executeSearch(processedQuery: ProcessedQuery, options: SearchOptions): Promise<SearchResult[]> {
    ...
    try {
      ...
      logger.error('Search execution failed', error instanceof Error ? error : new Error(String(error)));
      throw error;
    } finally {
      endTiming();
    }
  ...
  private async executeSemanticSearch(query: string, options: SearchOptions): Promise<SearchResult[]> {
    const searchOptions = {
    ...
    const vectorResults = await withRetry(
      async () => {
        return await withTimeout(
          this.vectorStore.search(query, searchOptions),
          {
            timeoutMs: 30000,
  ...
  private async executeKeywordSearch(query: string, options: SearchOptions): Promise<SearchResult[]> {
  ...
  private async executeHybridSearch(query: string, options: SearchOptions): Promise<SearchResult[]> {
    ...
    return this.fuseHybridResults(semanticResults, keywordResults, semanticWeight, keywordWeight, options.topK || 10);
  }

  private async fuseResults(results: SearchResult[], options: SearchOptions): Promise<SearchResult[]> {
  ...
  private async postProcessResults(results: SearchResult[], options: SearchOptions): Promise<SearchResult[]> {
    ...
    if (options.scoreThreshold) {
      processedResults = processedResults.filter(result => result.score >= options.scoreThreshold!);
    }
  ...
  private async fallbackSearch(query: string, options: SearchOptions): Promise<SearchResult[]> {
    ...
    try {
      ...
      logger.error('Fallback search also failed', error instanceof Error ? error : new Error(String(error)));
  ...
  private determineSearchStrategy(options: SearchOptions): 'semantic' | 'keyword' | 'hybrid' {
    if (options.useHybridSearch && this.config.search.enableHybridSearch) {
      return 'hybrid';
    } else if (options.useSemanticSearch !== false && this.vectorStore.capabilities.supportsMetadataFiltering) {
      return 'semantic';
    } else {
      return 'keyword';
    }
  ...
  private fuseHybridResults(
    ...
    for (const result of keywordResults) {
      ...
      if (existing) {
        // Combine scores
        const combinedScore = existing.hybridScore! + (result.keywordScore || result.score) * keywordWeight;
  ...
  private initializePipelines(): void {
    ...
    this.searchPipelines.set('keyword', new KeywordSearchPipeline(this.fileRepository, this.chunkRepository));
