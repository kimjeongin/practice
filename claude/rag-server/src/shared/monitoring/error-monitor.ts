...
export class ErrorMonitor extends EventEmitter {
  ...
  private checkAlertThresholds(error: StructuredError) {
    ...
    const recentErrors = this.errors.filter(e => 
      e.code === error.code && 
      e.timestamp.getTime() > timeWindow
    );
  ...
  getSystemHealth(): SystemHealth {
    ...
    const recentErrors = this.errors.filter(e => 
      now - e.timestamp.getTime() < 60 * 60 * 1000 // 1 hour
    );
    ...
    for (const [component, errorCount] of this.componentErrors.entries()) {
      const componentRecentErrors = recentErrors.filter(e => 
    ...
    if (errorRate > 10) {
      status = 'unhealthy';
    } else if (errorRate > 5 || Array.from(componentHealth.values()).some(c => c.status === 'unhealthy')) {
      status = 'degraded';
    }
   ...
   * 특정 에러 코드의 최근 발생 현황
   */
  getErrorTrend(errorCode: ErrorCode, hoursBack = 24): {
    count: number;
    ...
    const relevantErrors = this.errors.filter(e => 
      e.code === errorCode && e.timestamp.getTime() > cutoff
    );
    ...
    for (let i = 0; i < hoursBack; i++) {
      ...
      const hourErrors = relevantErrors.filter(e => 
        e.timestamp.getTime() >= hourStart && e.timestamp.getTime() < hourEnd
      );
