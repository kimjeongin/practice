...
import { VectorDocument, VectorSearchResult, VectorSearchOptions, IndexStats, FaissConfig } from '../core/types.js';
...
export class FaissProvider implements VectorStoreProvider {
  ...
  getIndexStats(): { total: number; occupied: number; sparsity: number; needsCompaction: boolean } | null {
    const sparsity = this.indexStats.total > 0 ? 
      (this.indexStats.total - this.indexStats.occupied) / this.indexStats.total : 0;
