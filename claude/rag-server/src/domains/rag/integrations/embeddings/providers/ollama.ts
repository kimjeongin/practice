...
export class OllamaEmbeddings extends Embeddings {
  ...
  async switchModel(modelName: string): Promise<void> {
    ...
    if (!availableModels[modelName]) {
      throw new Error(`Model ${modelName} is not available in Ollama. Available models: ${Object.keys(availableModels).join(', ')}`);
    }
    
    throw new Error(`Model switching not supported in current Ollama configuration. Current model: ${this.model}. To switch models, restart Ollama with the desired model.`);
  }
  ...
  async downloadModel(): Promise<void> {
    throw new Error('Model downloading should be done directly through Ollama CLI using `ollama pull <model-name>`');
  }
